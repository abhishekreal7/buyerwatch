...
-- Every customer-facing plan begins with the same card-verified seven-day
-- trial. Keep the acquisition allowance one-time per user and fail closed.
...
begin
  ...
  where id = p_user_id
    and plan in ('starter', 'pro', 'growth')
...
begin
  if new.auto_send_enabled = true then
    ...
    v_instant_trial := lower(coalesce(new.plan, 'free')) in ('starter', 'pro', 'growth')
...
begin
  ...
  if v_profile.plan not in ('starter', 'pro', 'growth')
...
-- Pick up any active seven-day Professional or Growth trials created before
-- this release without granting promotional monthly or annual subscriptions.
update public.profiles
set instant_autopilot_granted_at = coalesce(instant_autopilot_granted_at, billing_updated_at, now()),
    instant_autopilot_grant_event_id = coalesce(instant_autopilot_grant_event_id, 'all-plans-backfill:' || id::text),
    instant_autopilot_expires_at = coalesce(instant_autopilot_expires_at, billing_period_ends_at)
where plan in ('starter', 'pro', 'growth')
  and billing_status = 'active'
