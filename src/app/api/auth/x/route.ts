...
function errorRedirect(requestUrl: URL) {
  return NextResponse.redirect(
    new URL('/login?error=x_oauth_unavailable', requestUrl),
  )
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (error || !data.url) {
      console.error('[auth] X OAuth start failed:', error?.message || 'Missing provider URL')
      return errorRedirect(requestUrl)
    }

    return NextResponse.redirect(data.url)
  } catch (error) {
    console.error('[auth] X OAuth route failed:', error)
    return errorRedirect(requestUrl)
  }
}
