...
import { getSafeAttributionRedirectUrl } from '@/lib/security/outbound-url'
...
export async function GET(
  ...
  try {
    ...
    const destination = data?.destination_url
      ? getSafeAttributionRedirectUrl(data.destination_url)
      : null
    ...
    try {
      ...
      logger.warn(
        { code: trackingError instanceof Error ? trackingError.name : 'unknown' },
        'Short-link click tracking failed open',
      )
    }
